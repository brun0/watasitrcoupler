...
list_idParcel <- optiParams('paramfiles/', case_study_name, 'watasit_allIrrigatedCereals.csv', 'paramDBAllCereals.csv','climate_buech_2016-2017.csv', as.numeric(format(date_end_sim,"%Y")), 1, 365, 'NA'); warnings()
...
for (i in 1:length(list_idParcel)){
 ...
 I1   = as.vector(input_meteo$day) ; I1[] = 0;  I2   = as.vector(input_meteo$day); I2[] = 0 # I1 is surface irrigation and I2 is deep buried irrigation (I2 is null)
 #irr = rbind(irr,I1)
}
...
####### 3. Initialization of Optirrig model #######
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
list_idParcel <- optiParams('paramfiles/', case_study_name, 'watasit_allIrrigatedCereals.csv', 'paramDBAllCerealsItest1.csv','climate_buech_2016-2017.csv', as.numeric(format(date_end_sim,"%Y")), 1, 365, 'NA'); warnings()
...
for (i in 1:length(list_idParcel)){
  ...
  ####### 3.3 Create frame with all irrigation vectors  #######
  I1   = as.vector(input_meteo$day) ; I1[] = 1000; I2 []= 0 # I1 is surface irrigation and I2 is deep buried irrigation (I2 is null)
...
for (day in 2:dim(input_meteo)[1]){
  ...
  for (i in 1:length(list_idParcel)){
    ...
    optirday2 = daily_optirr(param,
                            ...
                            day) # Time step
    cat("I1[day] = ", I1[day],"\n","I2[day] = ", I2[day])
    if( I1[day] != 1000 ) {stop(' I1[day] ~= 1000')}
    # inval2 = optirday$inval ; inval2_list <- rbind(inval2_list, inval2) ; inval_list[i,] <- inval2 # New constants
    # vect2  = optirday$vect ; vect2_list <- rbind(vect2_list, vect2) ; vect_list[i,] <- vect2 # New vectors
    inval3 = optirday2$inval ; inval3_list[[i]] <- inval3 ; inval_list[i,] <- inval3 # New constants
    vect3  = optirday2$vect ; vect3_list[[i]] <- vect3 ; vect_list[i,] <- vect3 # New vectors
    lai_list[[i]] <- vect3$LAI; wsi_list[[i]]<- vect3$Sw_lai; etr_list[[i]]<-vect3$ETR
    etm_list[[i]]<-vect3$ETM; tt_list[[i]]<- vect3$TT; tt_p_list[[i]]<-vect3$TT_p; lai_p_list[[i]]<- vect3$LAI_p
    tdm_list[[i]]<- vect3$TDM; tdm_p_list[[i]]<- vect3$TDM_p
  }
} 
####### 4.1.3 Savings #######
