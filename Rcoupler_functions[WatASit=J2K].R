...
clean_workspace <- function() {
  ...
  rm(list=ls())
  # save starting time to measure execution time
  start_time <<- Sys.time();
  ...
  #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
simulatorSetup <- function(cormasOn = T, 
                           j2kOn = T, 
                           recordBalance = T,
                           saveResults = F,
                           hruPlots = F ) {
  # Active cormas N J2K during coupled simu
  with_cormas <<- cormasOn #TODO
  with_J2K <<- j2kOn # If set to false, only the warming up is performed and
  ...
  makeWaterBalance <<- recordBalance; if (makeWaterBalance) { storedWater <<- NULL; inOutWater <<-NULL}
  ...
  saveRes <<- saveResults #if False -> don't save results if True -> save results
}
...
initializeModels <- function(debug = )
