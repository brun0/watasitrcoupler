...
for (day in 2:dim(input_meteo)[1]){
  ...
  if (coupling == 1) { if (day >= DOY_start_coupling && day <= DOY_stop_coupling) { irriDailyDose <- RCormas(case_study_name = case_study_name, day = day, DOY_start_coupling = DOY_start_coupling, DOY_stop_coupling = DOY_stop_coupling, input_meteo = input_meteo, modelName = "COWAT", parcelFile = "WatASit[1.0].pcl", init = "INIT_2017_54x44", scenario = coupling_scenario) 
